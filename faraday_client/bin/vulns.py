...
__description__ = 'Show vulnerabilities similar to `vulns` metasploit'
__prettyname__ = 'Show vulnerabilities'
...
COLUMNS = {
    ...
    'vulns': lambda search, vuln, service, workspace: service.vuln_amount,
    'severity': lambda search, vuln, service, workspace: vuln.severity,
    'name': lambda search, vuln, service, workspace: color(vuln.severity) + vuln.name + Fore.RESET,
    'description': lambda search, vuln, service, workspace: color(vuln.severity) + grep(vuln.description, search) + Fore.RESET,
}
...
def get_default_columns():
    try:
        with open(path.join(CONF.getConfigPath(),'columns-vulns.txt'), 'r') as f:
            columns = f.read()
    except:
        columns = "host,port,service,severity,name"
    return columns

def set_default_columns(columns):
    with open(path.join(CONF.getConfigPath(),'columns-vulns.txt'), 'w') as f:
        f.write(columns)
...
def create_vuln(models, workspace, service, vuln):
...
def delete_vuln(workspace, vuln):
	models.delete_vuln(workspace, vuln.id)

def change_vuln(workspace, vuln, what):
...
def _len(col):
	return len(ansi_escape.sub('', str(col)))
...
def main(workspace='', args=None, parser=None):
	parser.add_argument('hosts', nargs="*", help='Vulnerability text', default=[])
	...
	parser.add_argument('-a', metavar='key=value', help='Add new Vulnerability', type=lambda opt: opt.split(","))
	parser.add_argument('-d', action='store_true', help='Delete Vulnerability', default=False)
	parser.add_argument('-e', action='append', metavar='key=value', help='Edit Vulnerability', type=lambda kv: kv.split("="))
	parser.add_argument('-S', type=str, metavar='text', help='Search Vulnerabilities', default="")
	parser.add_argument('--severity', metavar='text', help='Comma separated list of columns to show.',
                        default="critical,high,med,low,info,unclassified")
		...
		if not ip_list or host.getName() in ip_list:
			for service in host.getServices():
				if not port_list or set(port_list) & set(service.getPorts()):
					if parsed_args.a:
						create_vuln(models, workspace, service, dict([x.split('=') for x in parsed_args.a]))
						added += 1
					else:
						for vuln in service.getVulns():
							if vuln.severity in parsed_args.severity:
								if (not search or vuln.getDescription().lower().find(search.lower()) != -1 or vuln.getName().lower().find(search.lower()) != -1):
									if parsed_args.d:
										delete_vuln(workspace, vuln)
										deleted += 1
									elif parsed_args.e:
										change_vuln(workspace, vuln, dict(parsed_args.e))
										changed += 1
	...
	if added:
		print("added %d vulnerabilities"%added)
	elif deleted:
		print("deleted %d vulnerabilities"%deleted)
	elif changed:
		print("changed %d vulnerabilities"%changed)
	...
	for row in lines:
		for i in range(len(row)):
			col_widths[i] = _len(row[i]) if _len(row[i]) >= col_widths[i] else col_widths[i]
